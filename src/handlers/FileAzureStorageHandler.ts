...
import { AzureStorageUtility } from "@willsofts/will-store";
import { AZURE_STORAGE_PATH } from "../utils/EnvironmentVariable";
import path from "path";
import fs from "fs";

export class FileAzureStorageHandler extends PluginHandler {
    public storage : AzureStorageUtility | undefined;
    ...
    public override async perform(plugin: PluginSetting, context: KnContextInfo, model: KnModel = this.model) : Promise<[string,FileInfo | FileInfo[] | undefined]> {
        if(plugin) {
            ...
            if(plugin.connection && setting?.source && setting?.source.trim().length > 0) {
                ...
                //using url=connection string, database=bucket/container name
                this.storage = new AzureStorageUtility(connection?.url,connection?.database);
                ...
                if(filenames && Array.isArray(filenames)) {
                    if(filenames.length > 0) {
                        ...
                        for(let file of filenames) {
                            ...
                            let filepath = setting?.path || AZURE_STORAGE_PATH;  
