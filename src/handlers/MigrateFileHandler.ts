...
export class MigrateFileHandler extends MigrateTextHandler {
    ...
    public async file(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "file", raw: false}, this.doFile);
	}

    public async text(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "text", raw: false}, this.doText);
	}

    public async csv(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "csv", raw: false}, this.doText);
	}

    public async json(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "json", raw: false}, this.doJson);
	}

    public async excel(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "excel", raw: false}, this.doExcel);
	}

    public async xlsx(context: KnContextInfo) : Promise<MigrateRecordSet> {
        return this.callFunctional(context, {operate: "xlsx", raw: false}, this.doXlsx);
	}

    public async xml(context: KnContextInfo) : Promise<MigrateRecordSet> {
    ...
    protected async doText(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
        return await this.doInserting(context);
    }

    protected async doJson(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
        let handler = new MigrateJsonHandler();
        this.assignHandler(handler);
        return await handler.doInserting(context);
    }

    protected async doExcel(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
        let handler = new MigrateExcelHandler();
        this.assignHandler(handler);
        return await handler.doInserting(context);
    }

    protected async doXlsx(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
        let handler = new MigrateXlsxHandler();
        this.assignHandler(handler);
        return await handler.doInserting(context);
    }

    protected async doXml(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
        let handler = new MigrateXmlHandler();
        this.assignHandler(handler);
        return await handler.doInserting(context);
    }

    protected async doFile(context: KnContextInfo, model: KnModel) : Promise<MigrateRecordSet> {
